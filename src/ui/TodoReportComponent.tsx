...
function mapContainerToComponent(container: Container, deps: TodoReportComponentDeps) {
  ...
  const folded = false;
  return <div key={container.title} className="th-report-container">
    <h2 className="th-report-header">{container.title} <span onClick={() => {}}>{folded ? "▶" : "▼"}</span></h2>
    {!folded
...
export function TodoReportComponent({deps}: TodoReportComponentProps) {
  ...
  return <div className="th-report">
    <div>
      <h1><span>✅</span> Completed todos</h1>
    </div>
    <div>
