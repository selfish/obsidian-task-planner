...
export class TaskPlannerSettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		basicBuckets.forEach(bucket => {
			...
			checkbox.checked = this.plugin.settings.bucketVisibility[bucket.key as keyof typeof this.plugin.settings.bucketVisibility] as boolean;
			checkbox.addEventListener("change", async () => {
				(this.plugin.settings.bucketVisibility as unknown as Record<string, boolean>)[bucket.key] = checkbox.checked;
		...
		weekdays.forEach(day => {
			...
			const checkbox = checkboxWrapper.createEl("input", { type: "checkbox" });
			checkbox.checked = this.plugin.settings.bucketVisibility[day.key as keyof typeof this.plugin.settings.bucketVisibility] as boolean;
			checkbox.addEventListener("change", async () => {
				(this.plugin.settings.bucketVisibility as unknown as Record<string, boolean>)[day.key] = checkbox.checked;
				await this.plugin.saveSettings();
				await this.plugin.refreshPlanningViews();
				this.display();
			});
