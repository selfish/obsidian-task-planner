...
- Re-resolve indexed tasks against current vault content before mutations, fail safely when task identity is missing or ambiguous, and apply combined moves atomically per file with conflict-aware cross-file rollback while preserving untouched task metadata and line endings ([#120](https://github.com/selfish/obsidian-task-planner/issues/120)).
- Preserve historical completion dates across status undo/redo.
